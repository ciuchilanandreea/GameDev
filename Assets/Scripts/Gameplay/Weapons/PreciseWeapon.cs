...
{
    public class PreciseWeapon : GenericWeapon
    {
        Vector3 worldMousePos;
        Vector2 direction;
        private void Awake()
        {
            weaponCharacteristics.projectileSpeed = 6.5f;
            weaponCharacteristics.firingRate = 1.1f;
            StartCoroutine(Fire());
        }
        private void Update()
        {
        ...
        public override IEnumerator Fire()
        {
            while (true)
            {
                ...
                {
                    GenericProjectile projectile = Instantiate(this.projectile, transform.position +
                        (Vector3)(direction * 0.5f), Quaternion.identity);
